...
const teacherMenuItems: MenuItem[] = [
  ...
  { icon: School, label: "Quản lý lớp", path: "/teacher/management" },
  ...
  { icon: BarChart3, label: "Tiến độ học sinh", path: "/teacher/progress" },
];
...
const Sidebar = ({ isOpen, onClose }: SidebarProps) => {
  ...
  return (
    <aside
      className={`fixed left-0 top-0 md:top-[73px] bottom-0 w-64 flex flex-col bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 z-50 md:z-30 transition-transform duration-300 md:translate-x-0 ${isOpen ? "translate-x-0 shadow-2xl" : "-translate-x-full"}`}
    >
      {/* Logo */}
          ...
          <div className="mt-auto pt-4">
            <div className="rounded-2xl p-5 bg-gradient-to-br from-amber-100 to-orange-100 dark:from-amber-900/20 dark:to-orange-900/20 border border-amber-200/50 dark:border-amber-700/30 shadow-sm">
              ...
              <p className="text-sm text-slate-600 dark:text-slate-300">
                Tiếp tục phát huy, {user?.fullName?.split(" ")[0] || "bạn"}!
              </p>
